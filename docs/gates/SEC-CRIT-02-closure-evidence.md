...
| Commit | `1877b23` — `fix: close host credential replay by binding authority to private sessions` |
...
- `go test -race -count=1 -v ./...` on `go1.22.12 linux/amd64` → `ok meet-signal 1.177s`, 0 failures (`qa/reports/sec-crit-02/sec-02d-go122-race-results.txt`).

## Notes

- The annotated tag `sec-crit-02-closed` points at commit `1877b23`. This evidence addendum is committed after the tag as documentation; it does not alter the tagged candidate.
