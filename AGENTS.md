...
- **Windows env (this machine): `npm config script-shell` is `powershell` (v5)** — PS 5.1 rejects `&&`, so `npm run build` (`tsc ... && vite build`) fails with a parser error before executing. Do not report this as a build failure. Workaround: invoke directly — `cd poc/meet-webrtc-core && node node_modules/typescript/bin/tsc -p tsconfig.app.json && node node_modules/vite/bin/vite.js build` — or set `npm config set script-shell "C:\WINDOWS\system32\cmd.exe"`. Scripts without `&&` (`test`, `lint`, `load`) run fine via npm.
