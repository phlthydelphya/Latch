...
export async function initSW(): Promise<void> {
  ...
  try {
    // The production sw.js is generated by Workbox (vite-plugin-pwa, GenerateSW
    // strategy). It is a CLASSIC script: its UMD shim loads workbox-*.js via
    // importScripts(), which module-type service workers forbid by spec
    // ("Module scripts don't support importScripts()"). Never pass
    // { type: 'module' } here — classic is also the default, matching the
    // plugin-injected registerSW.js so both paths target one registration.
